...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS products
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  category TEXT, name TEXT, description TEXT, price INTEGER,
                  stock_status TEXT DEFAULT 'موجود',
                  image_link TEXT, models TEXT)''')
    c.execute('''CREATE TABLE IF NOT EXISTS orders
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  user_id INTEGER, username TEXT,
                  product_name TEXT, model TEXT, quantity INTEGER,
                  total_price INTEGER, name TEXT, phone TEXT,
                  address TEXT, postal_code TEXT,
                  tracking_code TEXT, status TEXT DEFAULT 'در انتظار تأیید')''')
    ...
    if c.fetchone()[0] == 0:
        for p in DEFAULT_PRODUCTS:
            c.execute("INSERT INTO products (category, name, description, price, stock_status, image_link, models) VALUES (?, ?, ?, ?, ?, ?, ?)",
                      (p['category'], p['name'], p['description'], p['price'], p['stock'], p['image_link'], p['models']))
    conn.commit()
    conn.close()
...
async def show_categories(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cats = get_categories()
    if not cats:
        await query.message.reply_text("✖️ هنوز محصولی اضافه نشده.")
        return
    keyboard = []
    for cat in cats:
...
async def show_category_products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not products:
        await query.message.reply_text("✖️ محصولی در این دسته پیدا نشد.")
        return
...
async def search_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("🔍 لطفاً نام محصول مورد نظرت رو وارد کن:")
...
async def handle_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not results:
        await update.message.reply_text("✖️ محصولی با این نام پیدا نشد.")
        return
...
async def stock_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("📦 لطفاً نام محصول رو وارد کن:")
...
async def handle_stock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if r:
        s = r[0]
        if s == "موجود":
            await update.message.reply_text("✅ موجود است.")
        elif s == "محدود":
            await update.message.reply_text("⚠️ تعداد محدود باقی مانده است.")
        else:
            await update.message.reply_text("❌ ناموجود است.")
    else:
...
async def handle_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if r:
        await update.message.reply_text(f"💰 قیمت روز: {r[0]:,} تومان")
    else:
        await update.message.reply_text("✖️ محصول پیدا نشد.")
...
async def order_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['mode'] = 'order_name'
    context.user_data['order'] = {}

async def order_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    name = update.message.text
    context.user_data['order']['product_name'] = name
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    ...
    if r and r[0]:
        ...
        await update.message.reply_text("🔢 لطفاً تعداد مورد نظر رو وارد کن:")
...
async def order_model_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['order']['model'] = model
    await query.message.reply_text("🔢 لطفاً تعداد مورد نظر رو وارد کن:")
    context.user_data['mode'] = 'order_qty'
...
async def order_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if r:
            total = r[0] * qty
            context.user_data['order']['total_price'] = total
            card_formatted = f"{CARD_NUMBER[:4]}-{CARD_NUMBER[4:8]}-{CARD_NUMBER[8:12]}-{CARD_NUMBER[12:]}"
            await update.message.reply_text(
                f"💰 مبلغ قابل پرداخت: {total:,} تومان\n\n💳 شماره کارت:\n`{card_formatted}`\n👤 به نام: {CARD_HOLDER}\n\n🏦 شبا:\n`{SHABA}`\n\n📸 لطفاً تصویر فیش واریزی رو بفرست.",
                parse_mode='Markdown',
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📋 کپی شماره کارت", callback_data=f"copy_{card_formatted}")]])
            )
...
async def copy_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("✅ شماره کارت کپی شد!", show_alert=True)

async def receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        await update.message.reply_text("📝 لطفاً نام و نام خانوادگی خودت رو وارد کن:")
        context.user_data['mode'] = 'info_name'
    else:
        await update.message.reply_text("✖️ لطفاً عکس فیش رو بفرست.")
...
async def info_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['mode'] = 'info_phone'
...
async def info_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['mode'] = 'info_address'
...
async def info_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['mode'] = 'info_postal'
...
async def info_postal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT INTO orders (user_id, username, product_name, model, quantity, total_price, name, phone, address, postal_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
              (update.message.from_user.id, update.message.from_user.username or "", o['product_name'], o.get('model', ''), o['quantity'], o['total_price'], o['name'], o['phone'], o['address'], o['postal_code']))
...
async def question_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("❓ سؤالت رو بنویس:")
...
async def track_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("🚚 لطفاً کد رهگیری سفارشت رو وارد کن:")
...
async def handle_track(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if r:
        await update.message.reply_text(f"📦 محصول: {r[0]}\n📋 وضعیت: {r[1]}")
    else:
        await update.message.reply_text("✖️ کد رهگیری نامعتبره.")
...
async def add_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✖️ فرمت: /add دسته | نام | توضیحات | قیمت | موجودی | مدل‌ها")
...
async def set_photo_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data['photo_for'] = pid
        await update.message.reply_text("📸 عکس رو بفرست.")
        context.user_data['mode'] = 'set_photo'
    except:
        await update.message.reply_text("✖️ مثال: /setphoto 1")
...
async def set_photo_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        if pid:
            ...
            path = f'{PHOTOS_DIR}/product_{pid}.jpg'
            await file.download_to_drive(path)
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("UPDATE products SET image_link = ? WHERE id = ?", (path, pid))
            conn.commit()
            conn.close()
...
async def change_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✖️ مثال: /price 1 | 36000000")
...
async def change_stock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if status not in ['موجود', 'محدود', 'ناموجود']:
            await update.message.reply_text("✖️ فقط: موجود / محدود / ناموجود")
            return
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("UPDATE products SET stock_status = ? WHERE id = ?", (status, pid))
        conn.commit()
        conn.close()
        await update.message.reply_text(f"✅ موجودی به «{status}» تغییر کرد.")
    except:
        await update.message.reply_text("✖️ مثال: /stock 1 | ناموجود")
...
async def set_tracking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        order = c.fetchone()
        if order:
            c.execute("UPDATE orders SET tracking_code = ?, status = 'ارسال شده' WHERE id = ?", (code, oid))
            conn.commit()
        ...
        await update.message.reply_text("✖️ مثال: /tracking 1 12345678")
...
async def answer_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        parts = update.message.text.split(' ', 2)
        qid, answer = int(parts[1]), parts[2]
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT user_id FROM questions WHERE id = ?", (qid,))
        r = c.fetchone()
        if r:
            c.execute("UPDATE questions SET answer = ?, status = 'closed' WHERE id = ?", (answer, qid))
            conn.commit()
            await context.bot.send_message(r[0], f"✅ پاسخ پشتیبانی:\n\n{answer}")
            await update.message.reply_text("✅ پاسخ ارسال شد.")
        else:
            await update.message.reply_text("✖️ سؤال پیدا نشد.")
        conn.close()
    except:
        await update.message.reply_text("✖️ مثال: /answer 1 سلام وقت بخیر")
...
async def adm_list_btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not products:
        await update.callback_query.message.reply_text("✖️ هنوز محصولی ثبت نشده.")
        return
    text = "📋 لیست محصولات:\n\n"
    for p in products:
        text += f"🆔 {p[0]} | 📂 {p[1]}\n📦 {p[2]}\n💰 {p[3]:,} تومان | 📦 {p[4]}\n\n"
...
async def adm_price_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.message.reply_text("💰 /price کد_محصول | قیمت_جدید")

async def adm_stock_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.message.reply_text("📦 /stock کد_محصول | موجود/محدود/ناموجود")

async def adm_photo_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.message.reply_text("🖼️ /setphoto کد_محصول\nسپس عکس رو بفرست.")
...
async def adm_orders_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not orders:
        await query.message.reply_text("✖️ هنوز سفارشی ثبت نشده.")
        return
    text = "📋 آخرین سفارشات:\n\n"
    for o in orders:
        text += f"🆔 {o[0]} | 👤 {o[1]}\n📦 {o[2]} | 💰 {o[3]:,} | {o[4]}\n\n"
...
async def adm_questions_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not qs:
        await query.message.reply_text("✖️ سؤال بازی وجود نداره.")
        return
    text = "❓ سؤالات باز:\n\n"
    for q in qs:
        text += f"🆔 {q[0]} | 👤 @{q[1]}\n💬 {q[2]}\n\n"
...
async def handle_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    handlers = {
        'search': handle_search, 'price': handle_price, 'stock': handle_stock,
        'order_name': order_name, 'order_qty': order_qty, 'receipt': receipt,
        'info_name': info_name, 'info_phone': info_phone, 'info_address': info_address,
        'info_postal': info_postal, 'track': handle_track, 'question': handle_question,
        'set_photo': set_photo_msg
    }
    if mode in handlers:
